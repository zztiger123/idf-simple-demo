...
void app_main(void)
{
    ...
    BLUFI_INFO("%s start test \n", __func__);

    while (1) {
        BLUFI_INFO("%s start to open blufi\n", __func__);

        ret = ble_start();

        if (0 == ret) {
            BLUFI_INFO("%s open blufi success , period = 30s\n", __func__);
        }

        vTaskDelay(30000 / portTICK_PERIOD_MS);

        BLUFI_INFO("%s start to close blufi\n", __func__);

        ret = ble_stop();

        if (0 == ret) {
            BLUFI_INFO("%s close blufi success , period = 30s\n", __func__);
        }
